...
from src.models import DraftRecord, DraftStatus, Profile, Scholarship
...
def _validate_safe_answer(*answers: str) -> None:
    ...
    for pattern in FORBIDDEN_CLAIM_PATTERNS:
        if re.search(pattern, combined, re.IGNORECASE):
            raise ValueError(f"Draft safety check rejected a forbidden claim pattern: {pattern}")
...
def generate_draft(
    ...
    *,
    ...
    _validate_safe_answer(material.draft, material.shorter, material.longer)
...
"""
    ...
    return DraftRecord(
        ...
        prompt=prompt,
        path=path,
        status=DraftStatus.NEEDS_USER_INPUT if missing else DraftStatus.DRAFT,
